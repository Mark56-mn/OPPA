...
export function createSecurityRouter(security: SecurityService) {
  ...
  router.post("/step-up/challenge", async (req, res, next) => {
    try {
      ...
      if (rawContext !== undefined && (!rawContext || typeof rawContext !== "object" || Array.isArray(rawContext))) throw new Error("SENSITIVE_CONTEXT_INVALID");
      const context: AuthorizationContext = rawContext ?? {};
