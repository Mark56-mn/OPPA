...
export class PostgresSecurityProofRepository implements SecurityProofRepository{
 private require(){if(!db)throw Error("DATABASE_URL is not configured");return db}
 ...
 async createChallenge(i:{userId:string;deviceId:string;purpose:StepUpPurpose;challengeHash:string;expiresAt:Date;maxAttempts:number}){const d=this.require();await d.query("update public.oppa_step_up_challenges set consumed_at=now() where user_id=$1 and purpose=$2 and consumed_at is null",[i.userId,i.purpose]);await d.query("insert into public.oppa_step_up_challenges(user_id,device_id,purpose,challenge_hash,expires_at,max_attempts) values($1,$2,$3,$4,$5,$6)",[i.userId,i.deviceId,i.purpose,i.challengeHash,i.expiresAt,i.maxAttempts])}
 async findActiveChallenge(userId:string,purpose:StepUpPurpose):Promise<StepUpChallenge|null>{const r=await this.require().query("select id,user_id as \"userId\",device_id as \"deviceId\",purpose,challenge_hash as \"challengeHash\" from public.oppa_step_up_challenges where user_id=$1 and purpose=$2 and consumed_at is null and expires_at>now() and attempts<max_attempts order by created_at desc limit 1",[userId,purpose]);return r.rows[0]??null}
