...
export class DefaultSensitiveAuthorization implements SensitiveAuthorization{
 ...
 async authorize(input:{userId:string;operation:SensitiveOperation;proof:AuthorizationProof}){
  await this.proofs.verify(input.userId,input.operation,input.proof);
  return true;
 }
}
