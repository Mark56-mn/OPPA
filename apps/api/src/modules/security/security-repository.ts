...
export type StepUpChallenge={id:string;userId:string;deviceId:string|null;purpose:StepUpPurpose;challengeHash:string;contextHash:string|null};
export interface SecurityRepository{
 isActiveDevice(userId:string,deviceId:string):Promise<boolean>;
 createChallenge(input:{userId:string;deviceId:string;purpose:StepUpPurpose;challengeHash:string;contextHash:string;expiresAt:Date;maxAttempts:number}):Promise<void>;
