...
export interface SensitiveAuthorization{
 authorize(input:{userId:string;operation:SensitiveOperation;proof:AuthorizationProof;intent?:SensitiveIntent}):Promise<boolean>;
}
...
export function isSensitiveOperation(value:string): value is SensitiveOperation{
 return value==="wallet_transfer"||value==="payment_reversal"||value==="security_change"||value==="account_recovery";
}