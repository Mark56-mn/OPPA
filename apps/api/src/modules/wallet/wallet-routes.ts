...
export function createWalletRouter(wallets: WalletRepository, transfers?: WalletTransferRepository, authorization?: SensitiveAuthorization) {
  const router = Router();
  router.get("/", async (req, res, next) => { try { res.json(await wallets.getOrCreate(req.auth!.userId)); } catch (e) { next(e); } });
  ...
  router.post("/transfer", async (req, res, next) => {
    try {
      ...
      const proof: AuthorizationProof = {
        ...
        context: { fromUserId: req.auth!.userId, toUserId, amountMinor, currency: "NGN", reference }
      };
