...
export class AuthService {
 constructor(private readonly otp:OtpService,private readonly identities:IdentityRepository,private readonly sessions:SessionService,private readonly devices:DeviceService){}
 async requestOtp(rawPhone:string){return this.otp.request(normalizePhone(rawPhone));}
 async verifyOtp(rawPhone:string,code:string,deviceId:string){
  ...
  if(existing&&!existing.phoneVerifiedAt)await this.identities.markPhoneVerified(existing.id,now);
