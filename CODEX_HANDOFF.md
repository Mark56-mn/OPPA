...
**§12 CALLS**: NEW **Call history** screen — real records from `GET /conversations/:id/calls`, aggregated across the caller's recent chats (bounded to 10 and the screen says so, because the API indexes history per conversation). Direction comes from a NEW server-projected `mine` flag (`callerUserId === caller`) added to the calls history route — the same pattern as message history — and "answered" from the server's `answeredAt`, so the client never infers who called whom. Filters: All / Missed / Outgoing / Incoming. NEW **Call settings** screen: states honestly that ring/answer/decline/hang-up are real and server-confirmed, that call AUDIO is not transmitted in V1 (so there is no quality to choose), and marks video + in-app ringtone as locked. Demo backend calls history now emits the production contract (`callerUserId`, `mine`, `answeredAt`, `endReason`, `startedAt`) and the demo-only `outgoing` key was REMOVED — a parity tripwire test now fails if it returns.
...
| `flutter test` | **72 pass / 0 fail** (+2 by-design skips) |
...
New tests (all green): `business_workspace_test.dart` (10) — creation persists and is listed, blank/oversized name refused with nothing created, a fresh store inherits no catalogue/orders and exactly one owner staff row, unknown id is a 404, add/edit/archive/restore product with documented error codes, rename persists, direct conversation is idempotent, per-store analytics isolation. `business_ui_test.dart` (3) — full merchant journey: switcher → create → lands in its own workspace with an empty catalogue; the created store is listed on the way back; the stored OPPA Look is restored on launch with the correct LIGHT brightness. `message_details_test.dart` (3) — receipts contract. `call_history_test.dart` (2) — call-history production contract (and the absence of the demo-only `outgoing` key) plus the Missed/Outgoing/Incoming filters. `startup_ui_test.dart` extended to walk all 8 onboarding steps including the real OPPA ID claim and the taken-name rejection (server-decided, Continue blocked). API: 3 new business tests (product edit incl. invalid price/status/empty patch, staff-only archived listing, owner-scoped profile rename).

**OUT OF SCOPE FOR THIS MISSION (stated, not silently skipped)**: the reference boards also show an Admin/Reports surface and a Web/Trust surface. This mission (§6-§19) is the Flutter mobile app; `web/` remains a single `index.html` trust page and the admin RBAC routes already exist server-side. Neither was touched here.
